...
export class CustomerOpenInvoicesComponent implements OnInit, OnDestroy {
  ...
  AddPaymentMain() {
    ...
    if (!this.selectedPaymentModeID) {
      this.toastService.showErrorToast('Error', 'Please select payment mode');
      return;
    }

    else {
      if (this.validatePassword(Type, paymentList.reduce((sum : any, current : any) => sum + current.dPaidAmount, 0))) { return; }
    }
    const Params = {
  ...
  calculateCustomerPayments(totalAmountAdded: number) {
    ...
    if ((totalAmountAdded >= 0) || (totalAmountAdded <= 0)) {
      ...
      // tslint:disable-next-line: max-line-length
      const allNegativeAmounts = this.AllCustomerOpenInvoices.filter((x : any) => x.dRemainingAmount < 0).reduce((sum : any, current : any) => sum + current.dRemainingAmount, 0) * -1;
      ...
      let allPositiveAmounts = this.customerSales.filter((x : any) => x.purchaseAmount > 0).reduce((sum : any, current : any) =>
