...
export class CustomerDirectPaymentComponent implements OnInit, OnDestroy {
  ...
  onSelectCustomerDataBinding(event: any) {
    let customer = this.AllCustomersList.find((x : any) => x.CustomerID == event.value);
  ...
  async UpdatePaymentMain() {
    ...
    if (IsBalancePayment) {
      if (this.EnteredBalance > 0) {
        this.toastService.showInfoToast('Info', "Amount Should be Less than 0 Or Negative Amount Should be Entered")
        return;
      }
    }
    else {
