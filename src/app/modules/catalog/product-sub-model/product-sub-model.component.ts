...
export class ProductSubModelComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('btn_open_modal_add_product_type') btn_open_modal_add_product_type: ElementRef;
  ...
  constructor(private apiService: vaplongapi,  private storageService: StorageService, private toastService: ToastService, private cdr: ChangeDetectorRef) {
  ...
  GetAllProductModelList() // Get All Product Quality Method Get Data from Service 
  {
    ...
    this.apiService.GetAllProductModels().pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseText === 'success') {
        ...
        this.filteredModels = response.ProductModels.filter((x: any) => x.ParentID == 0 && x.IsActive === true);
  ...
  AddProductModel()// Open Add New Product Model Section
  {
    this.ResetFields();
    this.IsAdd = true;
    this.btn_open_modal_add_product_type.nativeElement.click();
  }
  ...
  EditProductModel(productModel: ProductModel) {
    ...
    var pcategory: any = this.AllProductModelOriginalList.find(x=>x.ID==productModel.ParentID);
    this.selectedModel = {
      value: pcategory.ID,
