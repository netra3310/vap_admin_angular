...
import { Component, ElementRef, OnDestroy, OnInit, ViewChild, ChangeDetectorRef, SimpleChanges } from '@angular/core';
...
export class AddBackOrderComponent implements OnInit, OnDestroy {
  ...
  //       console.log('this.productdata is ', this.ProductData);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if(changes.addPurchase) {
      console.log("changed add purchase info is ", this.addPurchase);
    }
  ...
  AddPurchaseApiCall(data : any) {
    this.IsSpinner = true;
    const grandTotal = data.details.reduce((sum : any, current : any) => sum + Number(current.dTotalAmount), 0);
    const totalDiscount = data.details.reduce((sum : any, current : any) => sum + Number(current.dTotalDiscount), 0);
  ...
  AddProductToList() {
    ...
    console.log("search product id is ", this.productInfo[0].ProductID);
