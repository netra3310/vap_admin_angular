...
export class AddReceiptNewComponent implements OnInit, OnDestroy {
  ...
  orderByDropdown: any[] | any;
  filteredOrderBy: any[];
  selectedOrderBy: any;

  deliverToDropdown: any[] | any;
  ...
  isTextAreaDisable : boolean = false;
  ...
  GetAllProductStockList() {
    ...
    // this.apiService.GetAllProductStockListForSale(params).pipe(untilDestroyed(this)).subscribe((response: any) => {
      ...
      if (response.ResponseCode === 0) {
        ...
        if(this.usermodel.IsReseller)
        {
          response.AllProductVariantSaleList = response.AllProductVariantSaleList.filter((x : any) =>x.RemainingStock>0);
        }
  ...
  addMultipleProduct(event : any) {
    for (const item of event) {
      ...
      var index = this.cartProducts.findIndex((x : any) =>x.ProductID==item.ProductID);
      if(index!=-1)
      {
        this.toastService.showErrorToast("Error", this.cartProducts.find((x : any) =>x.ProductID==item.ProductID).selectedProductCart.label+ ' already exist in cart');
      }
      else
      {
  ...
  AddProductToList(): void {
    ...
    var index = this.cartProducts.findIndex((x : any) =>x.ProductID==this.productInfo[0].ProductID);

    if(index!=-1)
    {
      this.toastService.showErrorToast('error', this.cartProducts.find((x : any) =>x.ProductID==this.productInfo[0].ProductID).selectedProductCart.label+ ' already exist in cart');
      return;
    }
  ...
  OnChangeText(product: any) {
    if(this.usermodel.IsReseller)
      var productToCheck : any = this.AllProductListwithDiscountForSelection.filter((x : any) =>x.ProductID==product.ProductID && x.ID==product.ProductVariantID)[0];
    if(productToCheck!=null){
      if(product.Quantity>productToCheck.RemainingStock)
      {
        this.toastService.showErrorToast('error', 'Available stock of product is '+this.RemainingStock);
        product.Quantity = 1;
        return null;
      }
    }
  ...
  onChangeProductQuantity(selectedRow : any) {
    ...
    if(this.usermodel.IsReseller)
    var productToCheck : any = this.AllProductListwithDiscountForSelection.filter((x : any) =>x.ProductID==selectedRow.ProductID && x.ID==selectedRow.ProductVariantID)[0];
    if(productToCheck!=null){
        ...
        this.AllProductLocationList.forEach((item : any) => {
          ...
          if (autoAssignLocationWhenProduct) {
            ...
            if (selectedRow.IsAdded) {
              ...
              this.productInfo[0].SaleDetailNonTrackableLocations = this.productLocations;
              if (location1.Quantity <= 0) {    
                const indexno = this.productInfo[0].SaleDetailNonTrackableLocations.indexOf((x : any) =>
                  x.NonTrackableProductsLocationID == location1.NonTrackableProductsLocationID && x.LevelID == location1.LevelID);
                  if(indexno!=-1)
                  this.productInfo[0].SaleDetailNonTrackableLocations.splice(indexno, 1);
              }
              //this.productLocations = [];
            }
          }
  ...
  saveLocations(selectedRow : any) {
    ...
    for (const item of this.AllProductLocationList) {
      ...
      if (selectedRow.IsAdded) {
        ...
        // this code never executed as the above function only runs when the row is added and changed the location from the dialog text feild
        loc = this.productLocations.filter((x : any) =>
          x.NonTrackableProductsLocationID === location.NonTrackableProductsLocationID && x.LevelID === location.LevelID).shift();
        // tslint:disable-next-line: deprecation
        if (!(loc === null || loc === undefined)) {
  ...
  PrintingInvoiceFuntion(id : any) {
    ...
    this.apiService.GetPackingSlipByID(req).pipe(untilDestroyed(this)).subscribe((response1: any) => {
      if (response1.ResponseCode === 0) {
        ...
        this.printingData.txtSubTotal = response1.PackingSlip.PackingSlipDetails.reduce((sum : any, current : any) =>
  ...
  PrintReview() {
    setTimeout(() => {
      ...
      console.log("preview contents is ", printContents);
      popupWin = window.open('', '_blank', 'top=0,left=0,height=100%,width=auto');
      if(popupWin) {
        popupWin.document.open();
        popupWin.document.write(`
  ...
  ConfirmHold() {
    ...
    this.confirmationService.confirm('Do you want to temporary hold this sale?')
    .then((confirmed) => {
      if(confirmed) {
        this.HoldSale();
      }
        ...
        item.SaleDetailNonTrackableLocations.forEach((item1 : any) => {
          if (i === 0) {
            if (item1.Quantity > 0) {
              const row = {
                ProductVariantID: item.ProductVariantID,
                BLabel: this.AllProductListwithDiscountForSelection.find((x : any) =>x.ID==item.ProductVariantID)?.BLabel ?? "",
                Product: item.Product,              
